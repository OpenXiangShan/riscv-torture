...
case class Options(var outFileName: String = "test", var outDirName: String = "output",
...
object Generator extends App
{
  override def main(args: Array[String]) =
  {
    val parser = new OptionParser[Options]("generator/run") {
      ...
      opt[String]('d', "directory") valueName("<dir>") text("ouput directory") action {(s: String, c) => c.copy(outDirName = s)}
      opt[String]('o', "output") valueName("<filename>") text("output filename") action {(s: String, c) => c.copy(outFileName = s)}
    }
    parser.parse(args, Options()) match {
      case Some(opts) =>
        generate(opts.confFileName, opts.outDirName, opts.outFileName)
      case None =>
        System.exit(1) //error message printed by parser
    }
  }

  def generate(confFile: String, outDirName: String, outFileName: String): String = {
    ...
    generate(nseqs, memsize, fprnd, mix, vec, use_amo, use_mul, use_div, outDirName, outFileName, segment, loop, loop_size)
  }

  def generate(nseqs: Int, memsize: Int, fprnd : Int, mix: Map[String,Int], veccfg: Map[String,String], use_amo: Boolean, use_mul: Boolean, use_div: Boolean, outDirName: String, outFileName: String, segment : Boolean, loop : Boolean, loop_size : Int): String = {
